...
public class Experiments {
	...
	private static FeatureSpecification arcEagerSpec(final String lang,
														   final String corpus,
														   final boolean goldPos) {
		final FeatureSpecification spec = new FeatureSpecification();
		spec.setIntsFeature("prefixPoss", 0, 2);
		spec.setIntsFeature("suffixPoss", 0, 2);
		// spec.setIntsFeature("prefixForms", 0, 0);
		// spec.setIntsFeature("suffixForms");
		spec.setIntsFeature("prefixLemmas");
		spec.setIntsFeature("suffixLemmas");
		spec.setIntsFeature("leftDeprels", 0, 1);
		spec.setIntsFeature("rightDeprels", 0, 1);
		spec.setIntsFeature("parentPoss", 0, 1);
		spec.setIntsFeature("prefixLabels");
		spec.setGoldPos(goldPos);
		includeVecMappings(spec, lang, corpus);
		includePosTagger(spec, lang, corpus);
		return spec;
	...
	private static FeatureSpecification normalArcEagerSpec(final String lang,
														   final String corpus,
														   final boolean goldPos) {
		final FeatureSpecification spec = new FeatureSpecification();
		spec.setIntsFeature("prefixPoss", 0, 2);
		spec.setIntsFeature("suffixPoss", 0, 2);
		// spec.setIntsFeature("prefixForms", 0, 2);
		// spec.setIntsFeature("suffixForms", 0, 1);
		spec.setIntsFeature("prefixLemmas");
		spec.setIntsFeature("suffixLemmas");
		spec.setIntsFeature("leftDeprels", 0, 1);
	...
	public static ArcEagerExtractor trainArcEager(final String corpus,
												  final String corpusCopy,
												  final int n,
												  final FeatureSpecification spec,
		...
		final String deprelLeftFile = tmp + "deprelleftfile";
		final String deprelRightFile = tmp + "deprelrightfile";
		final ArcEagerExtractor extractor =
				new ArcEagerExtractor(spec, classifierFactory(), actionFile, deprelLeftFile, deprelRightFile);
		final double strayProb = 1.0;
		final int nIterations = 1;
		final SimpleTrainer trainer =
				dynamic ?
						new ArcEagerDynamicTrainer(spec,
						strayProb, nIterations, earlyReduce, strict, dynProjective) :
						new ArcEagerTrainer(spec, earlyReduce, strict);
	...
	public static RevisedArcEagerExtractor trainRevisedArcEager(final String corpus,
																final String corpusCopy,
																final int n,
																final FeatureSpecification spec,
																final boolean earlyReduce,
		...
		final String deprelLeftFile = tmp + "deprelleftfile";
		final String deprelRightFile = tmp + "deprelrightfile";
		final RevisedArcEagerExtractor extractor =
				new RevisedArcEagerExtractor(spec, classifierFactory(), actionFile, deprelLeftFile, deprelRightFile);
		//TMP
		final double strayProb = 1.0;
		final int nIterations = 1;
		final SimpleTrainer trainer =
				dynamic ?
						new RevisedArcEagerDynamicTrainer(spec,
						strayProb, nIterations, earlyReduce, strict, dynProjectivize) :
						new RevisedArcEagerTrainer(spec, earlyReduce, strict);
		trainer.setNonprojectiveAllowed(projectivize);
		final int nDone = trainer.train(corpus, corpusCopy, n, extractor);
		if (nDone != n)
			fail("" + corpus + ": processed " + nDone + " of " + n);
	...
	public static NormalArcEagerExtractor trainNormalArcEager(final String corpus,
															  final String corpusCopy,
															  final int n,
															  final FeatureSpecification spec,
															  final boolean dynamic,
															  ...
															  final boolean dynProjective) {
		final String actionFile = tmp + "actionfile";
		final String nilActionFile = tmp + "nilactionfile";
		final String deprelLeftFile = tmp + "deprelleftfile";
		final String deprelRightFile = tmp + "deprelrightfile";
		final NormalArcEagerExtractor extractor =
				new NormalArcEagerExtractor(spec, classifierFactory(), actionFile, nilActionFile,
						deprelLeftFile, deprelRightFile);
		final double strayProb = 1.0;
		final int nIterations = 1;
		final SimpleTrainer trainer =
				dynamic ?
						new NormalArcEagerDynamicTrainer(spec, strayProb, nIterations, dynProjective) :
						new NormalArcEagerTrainer(spec);
		trainer.setNonprojectiveAllowed(projectivize);
		final int nDone = trainer.train(corpus, corpusCopy, n, extractor);
		if (nDone != n)
			fail("" + corpus + ": processed " + nDone + " of " + n);
	...
	public static void trainTestArcEager(final String lang,
										 final String trainCorpus,
										 final String trainFile,
										 final String testCorpus,
										 final String goldFile,
									 ...
									 final boolean dynProjective,
									 final boolean goldPos) {
		final FeatureSpecification spec = arcEagerSpec(lang, trainCorpus, goldPos);
		final ArcEagerExtractor extractor = trainArcEager(trainCorpus, trainFile,
				nTrain, spec, earlyReduce, strict, dynamic, projectivize, dynProjective);
		final ArcEagerTester tester = new ArcEagerTester(spec, earlyReduce, strict);
		tester.test(testCorpus, goldFile, parsedFile, nTest, extractor);
	}

	public static void trainTestRevisedArcEager(final String lang,
											   final String trainCorpus,
												final String trainFile,
												final String testCorpus,
												final String goldFile,
									 ...
									 final boolean goldPos) {
		final FeatureSpecification spec = arcEagerSpec(lang, trainCorpus, goldPos);
		final RevisedArcEagerExtractor extractor = trainRevisedArcEager(trainCorpus, trainFile,
				nTrain, spec, earlyReduce, strict, dynamic, projectivize, dynProjectivize);
		final RevisedArcEagerTester tester = new RevisedArcEagerTester(spec,
				earlyReduce, strict);
		tester.test(testCorpus, goldFile, parsedFile, nTest, extractor);
	}

	public static void trainTestNormalArcEager(final String lang,
											   final String trainCorpus,
											   final String trainFile,
											   final String testCorpus,
											   final String goldFile,
											   ...
											   final boolean dynProjectivize,
											   final boolean goldPos) {
		final FeatureSpecification spec = normalArcEagerSpec(lang, trainCorpus, goldPos);
		final NormalArcEagerExtractor extractor = trainNormalArcEager(trainCorpus, trainFile,
				nTrain, spec, dynamic, projectivize, dynProjectivize);
		final NormalArcEagerTester tester = new NormalArcEagerTester(spec);
		tester.test(testCorpus, goldFile, parsedFile, nTest, extractor);
	}
	...
	public static void doTrainingAndTestingArcEager(final String[] langs,
													final int nTrain, final int nTest,
													final boolean earlyReduce,
													final boolean strict,
													final boolean dynamic,
		...
		for (String lang : langs) {
			final TimerMilli timer = new TimerMilli();
			timer.start();
			trainTestArcEager(lang, Corpora.trainFile(lang),
					tmp + "train-" + lang + ".conllu",
					Corpora.testFile(lang),
					tmp + "gold-" + lang + ".conllu",
					tmp + "parsed-" + lang + ".conllu",
					...
					earlyReduce,
					strict,
					dynamic, nonprojectiveAllowed,
					dynProjective, goldPos);
			timer.stop();
			final String report = "ArcEager earlyReduce=" + earlyReduce + " strict=" + strict + " " +
					lang + " took " + timer.seconds() + " s";
			reportFine(report);
			reportLogFile(report);
		}
	}

	public static void doTrainingAndTestingRevisedArcEager(final String[] langs,
														   final int nTrain, final int nTest,
														   final boolean earlyReduce,
														   final boolean strict,
														   final boolean dynamic,
		...
		for (String lang : langs) {
			...
			trainTestRevisedArcEager(lang,
					...
					earlyReduce,
					strict,
					dynamic, nonprojectiveAllowed, dynProjectivize, goldPos);
			timer.stop();
			final String report = "RevisedArcEager earlyReduce=" + earlyReduce + " strict=" + strict + " " +
					lang + " took " + timer.seconds() + " s";
			reportFine(report);
			reportLogFile(report);
		}
	...
	public static void doTrainingAndTestingNormalArcEager(final String[] langs,
														  final int nTrain, final int nTest,
														  final boolean dynamic,
														  final boolean nonprojectiveAllowed,
														  final boolean dynProjectivize,
		...
		for (String lang : langs) {
			final TimerMilli timer = new TimerMilli();
			timer.start();
			trainTestNormalArcEager(lang, Corpora.trainFile(lang),
					tmp + "train-" + lang + ".conllu",
					Corpora.testFile(lang),
					tmp + "gold-" + lang + ".conllu",
					tmp + "parsed-" + lang + ".conllu",
					...
					dynamic, nonprojectiveAllowed, dynProjectivize, goldPos);
			timer.stop();
			final String report = "NormalArcEager " + lang +
					" took " + timer.seconds() + " s";
			reportFine(report);
			reportLogFile(report);
		}
	...
	private static void doMethod() {
		...
		// final String method = "arceager";
		final String method = "revisedarceager";
		// final String method = "normalarceager";
		...
		else if (method.equals("arceager"))
			doTrainingAndTestingArcEager(langs, nTrain, nTest, earlyReduce, strict, dynamic,
					nonprojectiveAllowed, dynProjectivize, goldPos);
		else if (method.equals("revisedarceager"))
			doTrainingAndTestingRevisedArcEager(langs, nTrain, nTest, earlyReduce, strict, dynamic,
					nonprojectiveAllowed, dynProjectivize, goldPos);
		else if (method.equals("normalarceager"))
			doTrainingAndTestingNormalArcEager(langs, nTrain, nTest, dynamic,
					nonprojectiveAllowed, dynProjectivize, goldPos);
	...
	private static void testApproximation() {
		...
		doTrainingAndTestingRevisedArcEager(langs, nTrain, nTest, earlyReduce, strict, dynamic,
					nonprojectiveAllowed, dynProjectivize, goldPos);
		// double nEqualScoreObs = DataCollectionSum.globalCollection.nObservations("unequalScores", 0);
		// double nEqualActionObs = DataCollectionSum.globalCollection.nObservations("unequalActions", 0);
		double nEqualActionStringObs = DataCollectionSum.globalCollection.nObservations("unequalActions with two strings", 0);
		System.out.println("unequal scores " +
				DataCollectionSum.globalCollection.averagesString("unequalScores"));
		// System.out.println("unequal scores " + nEqualActionObs);
		System.out.println("unequal actions " +
				DataCollectionSum.globalCollection.averagesString("unequalActions"));
		// System.out.println("unequal actions " + nEqualScoreObs);

		// double nUnequalActions = DataCollectionSum.globalCollection.timings.get("unequalActions with two strings").get(0);
		// double nUnequalScores = DataCollectionSum.globalCollection.timings.get("unequalScores").get(0);
		LinkedList<String> sortedErrors = new LinkedList(DataCollectionSum.globalCollection.timings.keySet());
		Collections.sort(sortedErrors, (id1, id2) -> { return
			id1.startsWith("unequalActions ") && id2.startsWith("unequalActions ") ?
					DataCollectionSum.globalCollection.timings.get(id2).get(0).intValue() -
								DataCollectionSum.globalCollection.timings.get(id1).get(0).intValue() :
					0;
		});
		for (String id : sortedErrors) {
			if (id.startsWith("unequalActions ")) {
				double d = DataCollectionSum.globalCollection.timings.get(id).get(0);
				double perc = d * 100 / nEqualActionStringObs;
				System.out.println(
						String.format("%5." + 1 + "f", perc) + " " + id);
			}
		}
		// System.out.println(DataCollectionSum.globalCollection.observationStrings());
		System.out.println(
				DataCollectionSum.globalCollection.averagesStrings());
	}
	...
	private static void leaveParameters(final String method,
		...
		String params = method +
				...
				((method.equals("arceager") ||
						method.equals("revisedarceager")) &&
						!earlyReduce ? "-late" : "") +
				((method.equals("simple") && leftFirst ||
						method.equals("arceager") && earlyReduce ||
						method.equals("revisedarceager") && earlyReduce) &&
						strict ? "-strict" : "") +
				(dynamic ? "-dyn" : "") +
				(projectivize ? "-proj" : "") +
